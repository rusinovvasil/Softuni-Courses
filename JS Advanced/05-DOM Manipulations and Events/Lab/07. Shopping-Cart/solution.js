function solve() {
   ...
   function displayToCart({ name, price }) {
      html.output.innerHTML += `Added ${name} for ${price.toFixed(2)} to the cart.\n`
   }

   Array.from(html.products)
   .map(getSectionInfo)
   .forEach(x => 
      x.button.addEventListener("click", () => displayToCart(addToCart(x))))

      html.checkout.addEventListener("click", () => {
         html.output.innerHTML += `You bought ${[...state.names.keys()].join(", ")} for ${state.price.toFixed(2)}.`
